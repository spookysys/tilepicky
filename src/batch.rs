    ...
    pub fn ui(&mut self, ui: &mut eframe::egui::Ui, index: &Index, ai: &crate::ai::Ai, keys: &crate::ai::Keys) {
        ...
        if self.job.as_ref().is_some_and(Job::uncertain) {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Send again").on_hover_text("No batch was made. The provider may bill twice if one was.").clicked() {
        ...
        if self.running() {
            ui.horizontal(|ui| {
                if !self.error.is_empty() && ui.button("Try again now").clicked() { self.retry = true; }
                if ui.button("Cancel batch").on_hover_text("Asks the provider to cancel. Labels already saved stay.").clicked() {
                    self.cancel(keys);
                }
            });
        } else if self.job.is_some() && !self.error.is_empty() && ui.button("Try again now").clicked() {
            self.retry = true;
        }
