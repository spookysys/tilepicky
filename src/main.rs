...
impl App {
    ...
    fn assist_panel(
        ...
        if let Some(run) = run {
            ...
            cancel = Some(ui.button("Cancel").on_hover_text("Stops waiting. The provider may still bill the request."));
        ...
        let action = if label.clicked() { Some(labels::Action::Label) } else if remove.clicked() { Some(labels::Action::Remove) }
            else if cancel.as_ref().is_some_and(egui::Response::clicked) { Some(labels::Action::Cancel) } else { None };
        (vec![label, remove].into_iter().chain(cancel).collect(), action)
    }
